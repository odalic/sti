...
class SubjectColumnScorerHeuristic extends SubjectColumnScorer {
  ...
  @Override
  protected Map<Integer, Pair<Double, Boolean>> score(
      final List<TColumnFeature> featuresOfNEColumns) {
    final Map<Integer, Pair<Double, Boolean>> scores = new HashMap<>();
    // sort by column ids
    Collections.sort(featuresOfNEColumns, (o1, o2) -> {
      final int compared = new Integer(o1.getColId()).compareTo(o2.getColId());
      return compared;
    });

    // find max scores to calculate computeElementScores boosters
    final Map<String, Double> max_scores_for_each_feature = new HashMap<>();
    for (int index = 0; index < featuresOfNEColumns.size(); index++) {
      final TColumnFeature cf = featuresOfNEColumns.get(index);
      final double uc = USE_TOKEN_DIVERSITY ? cf.getUniqueTokenCount() + cf.getUniqueCellCount()
          : cf.getUniqueCellCount();
      final double cm = cf.getCMScore();
      final double ws = cf.getWSScore();
    ...
    for (int index = 0; index < featuresOfNEColumns.size(); index++) {
      ...
      final double cm = cf.getCMScore() * CM_WEIGHT;
      final double ws = cf.getWSScore();// * CM_WEIGHT; todo: check this, original code uses booster
      final double emc = cf.getEmptyCellCount() / (double) cf.getNumRows();

      int max_component_score_booster = 0;
      for (final Map.Entry<String, Double> e : max_scores_for_each_feature.entrySet()) {
      ...
      final Pair<Double, Boolean> score_object = new Pair<>(sum, f);
