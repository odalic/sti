...
/**
 * Created with IntelliJ IDEA. User: zqz Date: 27/02/14 Time: 12:43 To change this template use File
 * | Settings | File Templates.
 */
public class RelationBoWCreatorImpl implements OntologyBasedBoWCreator {
  @Override
  public List<String> create(String uri) {
    final int discardBegin = uri.indexOf("://");
    if (discardBegin != -1) {
      uri = uri.substring(discardBegin + 4).trim();
    }
    if (!uri.startsWith("/")) {
      final int discardEnd = uri.indexOf("/");
